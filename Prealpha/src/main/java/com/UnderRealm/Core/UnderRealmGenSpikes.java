...
public class UnderRealmGenSpikes extends WorldGenerator {
    //private int blockIndex;
	private Block blockType;

    //public UnderRealmGenSpikes(int par1)
    public UnderRealmGenSpikes(Block par1)
    {
        //this.blockIndex = par1;
    	this.blockType = par1;
    }
    
    private int RollMeta(Random par2Random)
    {
    	int blockMeta=0;
                ...
                for (j2 = 0; j2 < 16; ++j2)
                {
                    for (i2 = 0; i2 < 8; ++i2)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            if (i2 < 4 && !material.isSolid() && par1World.getBlock(par3 + i1, par4 + i2, par5 + j2) != this.blockType)
                            {
                                return false;
                            }
                        }
            ...
            for (i1 = 0; i1 < 16; ++i1)
            {
                for (j2 = 0; j2 < 16; ++j2)
                {
                    for (i2 = 0; i2 < 8; ++i2)
                    {
                        ...
                        {
                        	//par1World.setBlock(par3 + i1, par4 + i2, par5 + j2, this.blockIndex, this.RollMeta(par2Random), 2);
                        	par1World.setBlock(par3 + i1, par4 + i2, par5 + j2, this.blockType, this.RollMeta(par2Random), 2);
                        	
                            //X+Z+ random value / 2.
                            int xDist=Math.abs(i1-mid);
                            int yDist=Math.abs(j2-mid);
                            
                            int myDistance=xDist;
                            if (xDist < yDist) { myDistance=xDist; }
                            else			   { myDistance=yDist; }
                            
                        	HeightVarriance=16 - (myDistance + par2Random.nextInt(6) / 2);
                            for (int n = 0; n < HeightVarriance; n++)
                            {
                            	//par1World.setBlock(par3 + i1, par4 + i2 + n, par5 + j2, this.blockIndex, this.RollMeta(par2Random), 2);
                            	par1World.setBlock(par3 + i1, par4 + i2 + n, par5 + j2, this.blockType, this.RollMeta(par2Random), 2);
                            }
                        }
                    }
                }
